...
const PrivatePage = () => {
  ...
  const _onCreated = e => {
     ...
     let layer = e.layer;
     let group = [];
     console.log("item sayısı" + e.layers)
    //  layer = ([e.layer._latlng.lat, e.layer._latlng.lng ], [e.layer._latlng.lat, e.layer._latlng.lng ])
    //  console.log(layer);
     if (type === "marker") {
      // for(i = 0; i < sayac.length; i++){
      //   group.push(e.layer._latlng);
      //   console.log("grup okundu " + group )
      //   console.log("eleman sayısı"+group.length)
      // }
     

       console.log("Çizim oluşturuldu: Marker oluşturuldu", e.layer._latlng.lat); //console ekranına bakarak item çekebilirim.
