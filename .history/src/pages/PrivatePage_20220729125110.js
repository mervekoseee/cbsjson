...
const PrivatePage = () => {
  ...
  if (layerType === "marker") {
    var merkerlat = layer._latlng.lat;
    var markerlng = layer._latlng.lng;
    console.log("Çizim oluşturuldu: marker oluşturuldu.", markerlat);
  }
  ...
  console.log("Çizim oluşturuldu: kare oluşturuldu", layerType, layer._latlngs[0][0]);
   ...
   if(layer._latlngs[0][0].lat >= circlelat && layer._latlngs[0][0].lng >= markerlng){
    console.log("içeride")
    if(layer._latlngs[0][1].lat >= circlelat && layer._latlngs[0][1].lng >= markerlng){
      console.log("içeride")
      if(layer._latlngs[0][2].lat >= circlelat && layer._latlngs[0][2].lng >= markerlng){
        console.log("içeride")
        if(layer._latlngs[0][0].lat >= circlelat && layer._latlngs[0][0].lng >= markerlng){
          console.log("içeride")
         }
