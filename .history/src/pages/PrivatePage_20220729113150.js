...
const PrivatePage = () => {
  ...
  const _onCreated = e => {
    ...
    if (layerType === "circlemarker") {
      ...
      const {_leaflet_id} = layer;
       ...
       setMapLayers(layers => [
         ...layers, 
         {id: _leaflet_id, latLngs: layer.getLatLngs('lat')},
       ])
    ...
    //  }
      ...
      console.log("Çizim oluşturuldu: kare oluşturuldu", layerType, e.layer);
  ...
  return (
    <StyledPrivatePage>
      ...
      <pre className='text-topright'>{JSON.stringify(mapLayers,0)}</pre>
